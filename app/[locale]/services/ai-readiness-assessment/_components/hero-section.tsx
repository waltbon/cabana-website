import { Container } from "@/components/craft";
import { Button } from "@/components/ui/button";
import { FadeIn } from "@/components/animations";
import { getTranslations } from "next-intl/server";
import { ArrowRight } from "lucide-react";
import Link from "next/link";
import { PlasmaField } from "@/components/plasma-field";

export async function HeroSection() {
  const t = await getTranslations("services.rutaIA.hero");

  return (
    <section className="relative overflow-hidden py-2 md:pb-28 md:pt-2 text-center">
      {/* WebGL background */}
      <PlasmaField className="absolute inset-0 w-full h-full" />
      {/* Dark overlay */}
      <div className="absolute inset-0 bg-cabana-dark/70" />

      {/* Content */}
      <div className="relative z-10 flex min-h-[460px] items-center py-20 text-white">
        <Container className="max-w-5xl px-6">
          <div className="flex flex-col items-center gap-8 text-center">
            <FadeIn>
              <p className="text-sm font-semibold uppercase tracking-widest text-gradient-cabana">
                {t("tagline")}
              </p>
            </FadeIn>

            <FadeIn delay={0.1}>
              <h1 className="text-4xl font-bold leading-tight tracking-tight text-white md:text-5xl lg:text-[56px]">
                {t("headline")}{" "}
                <span className="text-gradient-cabana">{t("headlineHighlight")}</span>
              </h1>
            </FadeIn>

            <FadeIn delay={0.2}>
              <p className="max-w-2xl text-lg leading-relaxed text-white/80">
                {t("description")}
              </p>
            </FadeIn>

            <FadeIn delay={0.3}>
              <Button
                asChild
                size="lg"
                className="rounded-lg bg-cabana-blue px-8 hover:bg-cabana-blue/90"
              >
                <Link href="https://ruta-ai.cabanadata.com" target="_blank">
                  {t("cta")}
                  <ArrowRight className="size-4" />
                </Link>
              </Button>
            </FadeIn>
          </div>
        </Container>
      </div>
    </ section>
  );
}
